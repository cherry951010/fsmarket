...
require_once(ROOT_PATH . 'includes/www_mb5_com_cn.php');
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    /* 物流跟踪--开始  模板之家开发www。mb5.com.cn */
	if($order['shipping_id']!=-1 && $order['invoiceno'] && $order['shipping_name']){
...
/*------------------------------------------------------ */
//-- start一键发货 增加 by www.mb5.com.cn
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('prepare' == $operation)
    {
		...
		/* 商家配货时给客户发送短信提醒 */
		if ($_CFG['ecsdxt_order_picking'] == '1' && $order['mobile'] != '')
		{
			require_once(ROOT_PATH . 'includes/lib_sms.php');
