...
?>
      ...
      <div class="search-selected"> <span>您已选择：</span> 
		<?php if ($this->_var['filter_attr_sum'] > 0): ?> 
            <?php $_from = $this->_var['filter_attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'filter_attr_0_60973200_1541602066');if (count($_from)):
    foreach ($_from AS $this->_var['filter_attr_0_60973200_1541602066']):
?> 
                <?php $_from = $this->_var['filter_attr_0_60973200_1541602066']['attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'attr');if (count($_from)):
...
?>
          ...
          <?php $_from = $this->_var['filter_attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'filter_attr_0_61073200_1541602066');if (count($_from)):
    foreach ($_from AS $this->_var['filter_attr_0_61073200_1541602066']):
?>

          <?php if (! $this->_var['filter_attr_0_61073200_1541602066']['selected']): ?>
          <dl>
            <dt><?php echo htmlspecialchars($this->_var['filter_attr_0_61073200_1541602066']['filter_attr_name']); ?>：</dt>
            <dd>
              <div class="items cle"> 
                <?php $_from = $this->_var['filter_attr_0_61073200_1541602066']['attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'attr');if (count($_from)):
    foreach ($_from AS $this->_var['key'] => $this->_var['attr']):
?>
                 <?php if ($this->_var['key'] > 0): ?> 
