...

{
	...
	/* 代码增加_判断是否秒杀_start By www.0769web.net */
	$time_xg_now=gmtime();
	...
	if ( $row_xg['buymax'] >0 && $row_xg['sale_start_date'] < $time_xg_now  && $row_xg['sale_end_date'] > $time_xg_now  )
	{
		...
		{
			$result['error']  = 999;
			$result['message'] = "此商品为秒杀限购商品，需要登录后才能继续购买！";
			die($json->encode($result));
		}
		elseif ( intval($goods->number) > intval($row_xg['buymax']) )
		{
			$result['error']   = 888;
			$result['message'] = "此商品为秒杀限购商品，秒杀限购期间每人最多只能订购 ". $row_xg['buymax'] . " 件！";
			die($json->encode($result));
		}
		else
		{
	...
	/* 代码增加_判断是否秒杀_end  By www.0769web.net */
...
//-- 添加商品到购物车
/*------------------------------------------------------ */
elseif ($_REQUEST['step'] == 'add_to_cart_category')
{
    include_once('include/cls_json.php');
    $_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
			...
			$result['goods_id'] = $goods->goods_id;
            $result['content'] = insert_cart_flow_content();
	 ...
	 /* 计算合计 */
    $cart_goods = get_cart_goods_category();
...
elseif ($_REQUEST['step'] == 'drop_goods_category')
{
	...
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net

	$goods_id = $goods->goods_id;
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('cart'). " WHERE goods_id = '$goods_id' and $sql_where";
...
elseif ($_REQUEST['step']== 'ajax_update_cart_category')
{
    require_once(ROOT_PATH .'include/cls_json.php');
    $json = new JSON();
    $result = array('error' => 0, 'message'=> '');
	...
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
				...
				$sql = "SELECT count(rec_id) as rec_id FROM " .$GLOBALS['ecs']->table('cart'). " WHERE goods_id = '".$_POST['goods_id']."' and $sql_where";
				$row = $GLOBALS['db']->getRow($sql);
				if ($row)
				{
					if(intval($row['rec_id']) > 1){
						$result['error'] = 199;
						$result['goods_id'] = $_POST['goods_id'];
						$result['message'] = "本商品购买时选了多个规格\r\n\r\n请到购物车调整";            
						die($json->encode($result));
					}
				}


    if (isset($_POST['goods_id']) &&isset($_POST['goods_number']))
    {
		$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('cart'). " WHERE goods_id = '".$_POST['goods_id']."' and $sql_where";
		$row = $GLOBALS['db']->getRow($sql);//这里的缺陷是没有加入判断用户ID，最重要的是有规格的商品,会乱删除其中一个
		$key = $row['rec_id'];
	...
	/* 秒杀代码增加_start  By www.0769web.net */
	$time_xg_now=gmtime();
	...
	if ( $row['buymax'] >0 && $row['sale_start_date'] < $time_xg_now  && $row['sale_end_date'] > $time_xg_now  )
	{
		...
		{
			$result['error'] = 888;
			$result['message'] = "此商品为限购商品，限购期间每人最多只能订购 ". $row['buymax'] . " 件！";
			die($json->encode($result));
		}
		else
		{
			...
			if ( $num_total > intval($row['buymax']) )
			{
				...
				$message = "注意：\n\r 商品 【".$row['goods_name']."】  促销期间每人限购 ". $row['buymax'] . " 件\n\r";
				if ($num_cart_old)
				{
					$message .="您以前已经成功购买过 $num_cart_old 件！\n\r所以，";
				}
				$message .= "您最多只能再买 ". $num_else ." 件";
			$result['error'] = 888;
	...
	/* 秒杀代码增加_end  By www.0769web.net */
		...
		$sql_wherea = $_SESSION['user_id']>0 ? "a.user_id='". $_SESSION['user_id'] ."' " : "a.session_id = '" . SESS_ID . "' AND a.user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
		$sql_whereb = $_SESSION['user_id']>0 ? "b.user_id='". $_SESSION['user_id'] ."' " : "b.session_id = '" . SESS_ID . "' AND b.user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
        ...
        if ($val > 0)
        {
            ...
            if ($goods['extension_code'] =='package_buy')
            {
                ...
                $sql = "UPDATE ".$GLOBALS['ecs']->table('cart').
                        " SET goods_number= '$val' WHERE rec_id='$key' AND $sql_where";
            }
            /* 处理普通商品或非优惠的配件*/
            else
            {
                ...
                //更新购物车中的商品数量
                $sql = "UPDATE ".$GLOBALS['ecs']->table('cart').
                        " SET goods_number= '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND $sql_where";
            }
        }
        //订货数量等于0
        else
        ...
        $GLOBALS['db']->query($sql);
        /* 删除所有赠品*/
        $sql = "DELETE FROM " .$GLOBALS['ecs']->table('cart') . " WHERE $sql_where AND is_gift <> 0";
        $GLOBALS['db']->query($sql);
...
/* 增加购物车选择性结算--开始--青蜂网络www.0769web.net */
...
/* 增加购物车选择性结算--结束--青蜂网络www.0769web.net */

    /* 检查购物车中是否有商品 */
/* 【重写】增加购物车选择性结算--开始--青蜂网络www.0769web.net */
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        " WHERE $sql_where " .
        "AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type'";
/* 【重写】增加购物车选择性结算--结束--青蜂网络www.0769web.net */

    if ($db->getOne($sql) == 0)

    {

        show_message($_LANG['no_goods_in_cart'], '去选购商品', 'index.php', 'warning');
	...
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
    $sql = 'SELECT count(*) FROM ' . $ecs->table('cart') . " WHERE $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0";//修改购物车选择性结算--青蜂网络www.0769web.net
...
/* 增加购物车选择性结算--结束--青蜂网络www.0769web.net */
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        ...
        //" WHERE session_id = '" . SESS_ID . "' " .//青蜂网络购物车选择性结算注释
        " WHERE $sql_where " .//增加购物车选择性结算--青蜂网络www.0769web.net
...

    {
        ...
        //$cart_goods_stock = get_cart_goods();//青蜂网络购物车选择性结算注释
/* 增加购物车选择性结算--开始--青蜂网络www.0769web.net */
...
/* 增加购物车选择性结算--结束--青蜂网络www.0769web.net */
   ...
   /* 判断是不是实体商品 */
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
            ...
            //" WHERE session_id = '".SESS_ID."' AND rec_type = '$flow_type'";//青蜂网络购物车选择性结算注释
/* 增加购物车选择性结算--开始--青蜂网络www.0769web.net */
...

    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_number AS num FROM ".
               ...
               $GLOBALS['ecs']->table('cart') .
                ...
                //" AND session_id = '".SESS_ID."' AND rec_type = '$flow_type'";//青蜂网络购物车选择性结算注释
/* 增加购物车选择性结算--开始--青蜂网络www.0769web.net */
...
/* 增加购物车选择性结算--结束--青蜂网络www.0769web.net */
    ...
    /* 清空购物车 */
    //clear_cart($flow_type);//青蜂网络购物车选择性结算注释
    clear_cart($flow_type,$id_ext);//新增青蜂网络购物车选择性结算
...

{
...

    {
        ...
        $sql = "SELECT goods_name" .
                ...
                " WHERE $sql_where" .//修改购物车选择性结算--青蜂网络www.0769web.net
...

{
...

    {
        ...
        $goods_id = $db->getOne("SELECT  goods_id FROM " .$ecs->table('cart'). " WHERE rec_id = '$rec_id' AND $sql_where ");//修改购物车选择性结算--青蜂网络www.0769web.net
...
{
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
    $sql = "SELECT SUM(g.integral * c.goods_number) ".

            "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " .
            //"WHERE c.session_id = '" . SESS_ID . "' AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 " .//青蜂网络购物车选择性结算注释
            "WHERE $sql_where AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 " .//增加购物车选择性结算--青蜂网络www.0769web.net
...

{
...

    {
        ...
        $sql = "SELECT `goods_id`, `goods_attr_id`, `product_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart').
               ...
               " WHERE rec_id='$key' AND $sql_where";//修改购物车选择性结算--青蜂网络www.0769web.net
...
/* 代码修改购物车选择性结算_start  By  www.0769web.net  将这块替换掉*/
...
/* 代码修改购物车选择性结算_end  By  www.0769web.net  */
...

        {
...

            {
...

                {
                    ...
                    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
                            ...
                            " WHERE $sql_where1 " .//修改购物车选择性结算--青蜂网络www.0769web.net
...

            {
                ...
                $sql = "UPDATE " .$GLOBALS['ecs']->table('cart').
                        ...
                        //" SET goods_number = '$val' WHERE rec_id='$key' AND session_id='" . SESS_ID . "' AND group_id=''"; //青蜂网络购物车选择性结算注释
                        " SET goods_number = '$val' WHERE rec_id='$key' AND group_id='' AND $sql_where1"; //增加购物车选择性结算--青蜂网络www.0769web.net
...

            {
                ...
                $sql = "UPDATE " .$GLOBALS['ecs']->table('cart').
                        ...
                        //" SET goods_number = '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND session_id='" . SESS_ID . "' AND group_id=''"; //青蜂网络购物车选择性结算注释
                        " SET goods_number = '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND group_id='' AND $sql_where1"; //增加购物车选择性结算--青蜂网络www.0769web.net
...

        {
...

            {
                ...
                $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
                        ...
                        //" WHERE session_id = '" . SESS_ID . "' " .//青蜂网络购物车选择性结算注释
					    " WHERE $sql_where1 " .//增加购物车选择性结算--青蜂网络www.0769web.net
            ...
            $sql = "DELETE FROM " .$GLOBALS['ecs']->table('cart').
                ...
                //" WHERE rec_id='$key' AND session_id='" .SESS_ID. "'"; //青蜂网络购物车选择性结算注释
                " WHERE rec_id='$key' AND $sql_where1"; //增加购物车选择性结算--青蜂网络www.0769web.net
    ...
    //$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id = '" .SESS_ID. "' AND is_gift <> 0";//青蜂网络购物车选择性结算注释
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE $sql_where1 AND is_gift <> 0";//修改购物车选择性结算--青蜂网络www.0769web.net
...

{
    ...
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//新增青蜂网络购物车选择性结算

/* 修改购物车选择性结算--结束--青蜂网络www.0769web.net  将这块替换掉*/
    $sql = "SELECT c.rec_id, gg.parent_id
            ...
            AND gg.parent_id > 0
            AND g.is_alone_sale = 0";
/* 修改购物车选择性结算--结束--青蜂网络www.0769web.net */
...
{
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//增加购物车选择性结算--开始--青蜂网络www.0769web.net
    /* 查询优惠范围内商品总额的sql */
    ...
    $sql = "SELECT SUM(c.goods_price * c.goods_number) " .
			...
			//"AND c.session_id = '" . SESS_ID . "' " .//青蜂网络购物车选择性结算注释
            "AND $sql_where " .//增加购物车选择性结算--青蜂网络www.0769web.net
