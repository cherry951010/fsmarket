...
if ($action == 'default')
{
    ...
    $sqlimg = 'SELECT headimg FROM ' . $ecs->table('users') . " WHERE user_id = $user_id";
    $headimg = $db->getOne($sqlimg);
	...
	if(!empty($wxid)){
		...
		$info['avatar'] = $config['site_url'] . $headimg;
	}
	/*青蜂网络(www.0769web.net)分销微分销新增显示分销会员标准*/
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$level_register_up = (float)$affiliate['config']['level_register_up'];

	$sql="SELECT count(*) as order_num ,sum(goods_amount - discount)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$_SESSION["user_id"]." and  pay_status=2 and shipping_status = 2";
	...
	/*青蜂网络(www.0769web.net)分销微分销新增查询佣金*/
	//推荐注册分成
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$user_list['user_list'] = array();
	$auid = $_SESSION['user_id'];
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
			...
			foreach($user_info as $key=>$value){
				/*青蜂网络(www.0769web.net)分销微分销开发未付款的佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount,sum(goods_amount) as goods_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=0 GROUP BY user_id";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				...
				/*青蜂网络(www.0769web.net)分销微分销开发已经付款且未收货佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount,goods_amount,sum(goods_amount) as goods_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=2 and   shipping_status  <> 2 GROUP BY user_id";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				$setmoney = round($order_info['order_amount'] * $affiliate['item'][$k]['level_money'], 2);
				...
				/*青蜂网络(www.0769web.net)分销微分销开发已经收货佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount,goods_amount,sum(goods_amount) as goods_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']."  and  shipping_status  =2 GROUP BY user_id";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				$setmoney = round($order_info['order_amount'] * $affiliate['item'][$k]['level_money'], 2);
...

{
...

	{
		...
		$url = $c->login($ecs->url().'accountbind.php?act=oath_login&type='.$type.'&callblock='.urlencode($back_act).'&open='.$open);
...

{
    ...
    $smarty->display('user_passport_distribute.dwt');
...

{
	...
	//青蜂网络(www.0769web.net)分销显示会员等级和下面人数
...

{
...

    {
...

        {
            ...
            $sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,wxid,headimg ".
	...
	foreach($user_list['user_list'] as $key =>$value){
		...
		if($value['level']==1){
			...
			if($value['head_url']){
				$value['head_url'] = $value['head_url'];
			}elseif($value['headimg']){
				$value['head_url'] = $config['site_url'] . $value['headimg'];
			}
...
//青蜂网络(www.0769web.net)分销微分销开发结束
...

    {
...

        {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    ...
                    " WHERE o.user_id > 0 AND o.fencheng > 0 AND (u.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd";
...

        {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    ...
                    " WHERE o.user_id > 0 AND o.fencheng > 0 AND (o.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd";
