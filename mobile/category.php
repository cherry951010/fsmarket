...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update', 'click_count', 'sales_volume_base'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type; // 增加按人气、按销量排序 by www.0769web.net
...
$display  = in_array($display, array('list', 'grid', 'album')) ? $display : 'album'; // by www.0769web.net
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    {
        $temp_key = $key + 1;
        $brands[$temp_key]['brand_id'] = $val['brand_id']; //同步绑定品牌名称和品牌ID by www.0769web.net
        ...
        /* 判断品牌是否被选中 */
        if ($brand == $val['brand_id']) //修正当前品牌的ID by www.0769web.net
        {
    ...
    unset($brands[0]); //清空索引为0的项目 by www.0769web.net
    $brands[0]['brand_id'] = 0; //新增默认值 by www.0769web.net
    ...
    ksort($brands); //增加再排序 by www.0769web.net
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            {
                $all_attr_list[$key]['filter_attr_id'] = $value; //新增属性标识 by www.0769web.net
                ...
                $all_attr_list[$key]['attr_list'][0]['attr_id'] = 0; //默认数值 by www.0769web.net
                ...
                foreach ($attr_list as $k => $v)
                {
                    ...
                    $all_attr_list[$key]['attr_list'][$temp_key]['attr_id'] = $v['goods_id']; //新增属性参数 by www.0769web.net
    ...
    if ($_GET['act'] == 'asynclist') {
        ...
        if (is_array($goodslist)) {
            foreach ($goodslist as $vo) {
				$cart_goods_id = $vo['goods_id'];
				$sql = "SELECT rec_id,sum(goods_number) as goods_number FROM " .$GLOBALS['ecs']->table('cart'). " WHERE goods_id = '$cart_goods_id' and session_id = '" . SESS_ID . "' GROUP BY rec_id";
				$row = $GLOBALS['db']->getRow($sql);
				if ($row)
				{
					$cart_goods_number = intval($row['goods_number']);
					$cart_rec_id = $row['rec_id'];
				}else{
					$cart_goods_number = 0;
					$cart_rec_id = 0;
				}
        ...
        <div class="proInfo-wrap">
          ...
          <div class="proSales">已售出<em><span class="sales">'.$vo['sales_volume_base'].'</span></em></div>
		  <a href="javascript:;" onclick="categoryaddToCart2('.$vo['goods_id'].')" class="catbuybtn"></a>
		</div>'
                );
            }
        }
    ...
    /*
     * 异步显示商品列表 by www.0769web.net end
     */
