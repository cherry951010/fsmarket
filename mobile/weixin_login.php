...
if(empty($code))
{
  show_message('scope参数错误或没有scope权限11' , '首页',$ecs->url() , 'error');
}
...
if(empty($info['name']))
{
  show_message('scope参数错误或没有scope权限22' , '首页',$ecs->url() , 'error');
}
...
if (!$info['user_id']) {
  ...
  $count = $db->getRow($sql);
  if (!$count) {   // 没有当前数据
	  require_once(ROOT_PATH . 'lang/' .$_CFG['lang']. '/common.php');
	  if ($_CFG['shop_reg_closed']){
		  show_message($_LANG['shop_register_closed'], $_LANG['back_up_page'], 'user.php', 'info');
	  }
