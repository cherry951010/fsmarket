...
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','send_pwd_mobile','password', 'signin', 'add_tag', 
    ...
    'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name','mpassword_name', 'get_passwd_question', 'check_answer', 'oath');



/* 显示页面的action列表 */

$ui_arr = array('register', 'login', 'profile','headimg', 'dianpu', 'act_dianpu', 'order_list', 'order_detail', 'order_tracking', 'address_list', 'act_edit_address', 'collection_list',

'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',

'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name','mpassword_name','send_pwd_mobile', 'get_passwd_question', 'check_answer','point','user_card','fenxiao1','myorder','myorder_detail','fenxiao2','fenxiao3','fenxiao4','bindmobile', 'back_order', 'back_list', 'back_order_detail', 'back_order_act', 'back_replay', 'del_back_order', 'back_order_detail_edit', 'qrcode');
...

{
	...
	if(!empty($wxid)){
		...
		if(!empty($weixinInfo['headimgurl']))
		{
		...
		{
			$info['avatar'] =  $headimg; // $config['site_url'] .$headimg;
		}
	}elseif($headimg){
		$info['avatar'] =  $headimg; // $config['site_url'] .$headimg;
	}
	...
	if($rank_points>$level_register_up||$rank_points==$level_register_up){
		...
		$xkfla="分销商：否     <a href='./'>点此链接购买成为分销商</a>";
...

{
	...
	if($c)
	{
		...
		if( $type=='weixin' ) {
			...
			$url ='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.WX_AKEY.'&redirect_uri='.urlencode($ecs->url().'accountbind.php?act=oath_login&type='.$type).'&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
...

{
...

    {
        ...
        $email    = isset($_POST['email']) ? trim($_POST['email']) : $username.'@mail.com';
...
elseif ($action == 'qrcode')
{
	...
	$shopurl = str_replace('mobile/', '', $ecs->url());
	$smarty->assign('shopurl', $shopurl);
	$wxid  = $db->getOne("SELECT wxid FROM " .$ecs->table('users'). " WHERE user_id='$user_id'");
	$affiliate = unserialize($_CFG_MOBILE['affiliate']);
	$level_register_up = (float)$affiliate['config']['level_register_up'];
	$sql="SELECT count(*) as order_num ,sum(goods_amount - discount)  as order_amount FROM ".$ecs->table('order_info')."WHERE user_id=".$user_id." and  pay_status=2 and shipping_status  = 2";
	$order_info=$db->getRow($sql);
	$rank_points=$order_info['order_amount'];
	if(round($rank_points)<round($level_register_up)){
		show_message('您还不是分销商，暂时不能获取推广二维码', $_LANG['back_page_up'], '', 'info');
	}else{
		if (is_file('../images/qrcode/qrcode_200/' . $wxid .'.jpg')){
			$qrcodeimg = 'images/qrcode/qrcode_200/' . $wxid .'.jpg';
		}else{
			$qrcodeimg= 'erweima_user.php?id='.$user_id;
		}
	}
	$smarty->assign('qrcodeimg', $qrcodeimg);
    $smarty->display('user_clips.dwt');
}
...
elseif ($action == 'act_edit_headimg')
{
	...
	$headimg_path = '../' . DATA_DIR . '/headimg/' . date('Ym') . '/';
	
	$headimg_thumb = $image->make_thumb($headimg_original, '150', '150', $headimg_path);
	
	$headimg_thumb = str_replace(array("../"),"",$headimg_thumb);
	@unlink($old_headimg);
