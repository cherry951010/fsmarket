...
/* 增加购物车选择性结算--结束--青蜂网络www.0769web.net */
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            ...
            " AS g ON c.goods_id = g.goods_id WHERE $sql_where " .//修改青蜂网络购物车选择性结算where条件
            "AND rec_type = '$type' $id_ext ";//修改青蜂网络购物车选择性结算$id_ext
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart') .
                " WHERE goods_id='$parent' AND $sql_where AND extension_code <> 'package_buy'";//修改购物车选择性结算--青蜂网络www.0769web.net
    ...
    if ($num > 0)
    {
       ...
       $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                " WHERE $sql_where AND goods_id = '$goods_id' ".//修改购物车选择性结算--青蜂网络www.0769web.net
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE $sql_where AND is_gift <> 0";//修改购物车选择性结算--青蜂网络www.0769web.net
    $GLOBALS['db']->query($sql);
...
function addto_cart_combo($goods_id, $num = 1, $spec = array(), $parent = 0, $group = 0)
{
    ...
    if ($parent > 0)
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart_combo') .
                " WHERE goods_id='$parent' AND $sql_where AND extension_code <> 'package_buy' AND group_id = '$group'";//修改购物车选择性结算--青蜂网络www.0769web.net
        if ($GLOBALS['db']->getOne($sql) == 0)
        {
            $GLOBALS['err']->add($GLOBALS['_LANG']['no_basic_goods'], ERR_NO_BASIC_GOODS);
...
//        $num -= $parent['goods_number'];
    ...
    if ($_parent_id <= 0) 
    {
        ...
        {
            //添加基本件的同时清空该基本件下的配件
            $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE $sql_where".//修改购物车选择性结算--青蜂网络www.0769web.net
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE $sql_where AND is_gift <> 0";//修改购物车选择性结算--青蜂网络www.0769web.net
    $GLOBALS['db']->query($sql);
 ...
 * @param   int     $type   类型：默认普通商品
 */
//function clear_cart($type = CART_GENERAL_GOODS)//青蜂网络购物车选择性结算注释
function clear_cart($type = CART_GENERAL_GOODS,$other='')//新增青蜂网络购物车选择性结算
{
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' ";//新增青蜂网络购物车选择性结算
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            //" WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type'";//青蜂网络购物车选择性结算注释
/* 增加购物车选择性结算--开始--青蜂网络www.0769web.net */
 ...
 * @return  array
 */
//function get_cart_goods()//青蜂网络购物车选择性结算注释
function get_cart_goods($other='')//新增青蜂网络购物车选择性结算
{
    /* 初始化 */
    $goods_list = array();
    $total = array(
    ...
    /* 循环、统计 */
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//新增青蜂网络购物车选择性结算
    $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid , goods_number " .
            ...
            " WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "' $other " .//新增青蜂网络购物车选择性结算
...
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
    if ($order_id <= 0)
    {
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart') .
                " WHERE $sql_where AND is_real = 1 " .//修改购物车选择性结算--青蜂网络www.0769web.net
