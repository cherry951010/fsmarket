...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<li>
              <div class="p-img"><a target="_blank" href="'.$goods['url'].'"><img data-original="'.$goods['goods_thumb'].'" src="images/spacer.gif" class="loading" alt="'.$goods['goods_name'].'"></a></div>
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
           ...
           if ($row['promote_price'] > 0)
			...
			$str .= '
					 <li> 
					  <a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'"><img data-original="'.$goods['goods_thumb'].'" src="images/spacer.gif" class="loading" alt="'.$goods['goods_name'].'"></a>	
...
{
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//青蜂网络www.0769web.net购物车选择性结算注释
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                ...
                target='_blank'><img data-original='$src' src='images/spacer.gif' class='loading' width='" .$row['ad_width']. "' height='$row[ad_height]'
                border='0' /></a>";
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
